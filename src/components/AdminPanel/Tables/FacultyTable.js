              ...
              <br></br>
                ...
                <br></br>
                {
                  data.fileUrl.map((url,index)=>{
                    return(
                      <><a target={'_blank'} href={url}>File{index+1}</a><br></br></>
                    )
                  })
                }
            </Modal.Body>
...
export default function DataTable(props) {
  ...
  // GET ALL DOCUMENT FOR FACULTY DATA (WITHOUT TIMESPAN)
  const getDocumentData = async () => {
    let q;
    q = query(userDataCollectionRefDocumentData, orderBy("date", "desc"), where("uploads", "==", "Faculty"));
...
// GET DOCUMENT IN TIME SPAN FOR FACULTY DATA
const GetDocumentInTimeSpan = async (e) => {
    ...
    if (startDate === "" && endDate === "") {
      ...
      let q;
      q = query(userDataCollectionRefDocumentData, where('date', '>=', startDate), where('date', '<=', endDate), where('uploads','==','Faculty'));
      const data = await getDocs(q);
  ...
  useEffect(() => {
    // getDocumentData();
    // console.log(props.documentData.filter(dataa => dataa.uploads === "Faculty"));
    setDocumentData(props.documentData.filter(dataa => dataa.uploads === "Faculty"));
  }, [])
                ...
                <React.Fragment>
                  <Box component="form" noValidate sx={{ mt: 3 }}>
                    <Grid container spacing={2}>
                      ...
                      <Grid item xs={12}>
                        <TextField type="date" onChange={(e) => { SetDate(e.target.value) }} value={date} id="datein" required
                            fullWidth></TextField>
                      </Grid>
                      <Grid item xs={12}>
                        <FormControlLabel
